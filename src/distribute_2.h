...
*/
	...
	return client.info;
	 ...
	 std::ofstream do_file;
        do_file.open("/home/wcw/dmclock-log/do_add_request.txt", std::ios::app);
        if (do_file.bad()) {
	...
	do_file << " Idle, inital lowest_prop_tage_trigger is " << format_time(lowest_prop_tag_trigger) <<"lowest_prop_tag is " << lowest_prop_tag <<"\n";
	  for (auto const &c : client_map) {
	    // don't use ourselves (or anything else that might be
	    // listed as idle) since we're now in the map
	    // c.second is the ClientRec reference
	    if (!c.second->idle) {
	      double p;
		...
		p = c.second->next_request().tag.proportion +
		  c.second->prop_delta;
		do_file << "has_request, p: " << format_time(p, 1000000) << "\n";
	      } else {
		// use the prev_tag
	        p = c.second->get_req_tag().proportion + c.second->prop_delta;
		do_file << "use the prev_tag, p: " << format_time(p, 1000000) << "\n";
	      }

	      if (p < lowest_prop_tag) {
		lowest_prop_tag = p;
		do_file << "update lowest_prop_tag \n";
	      }
	    		first_idle = false;
		}
	  }// find the lowest_prop_tag amoung all clients

	  // if this conditional does not fire, it
	  if (lowest_prop_tag < lowest_prop_tag_trigger) {
	// cause time might be smaller than prev, so the next w_tag may not be time.
//	    client.prop_delta = lowest_prop_tag - time;
	    RequestTag tmp_tag(0, 0, 0, time);
	// here when client is idle, consider it is in R phase w_tag = max(prev, time)
	// when R = 0, this should be in W phase, w_tag should be max(prev + 1/w,time)
	...
	const ClientInfo* tmp_client_info = get_cli_info(client);
	  ...
	  tmp_tag = RequestTag(true, win_start,client.get_req_tag(), *tmp_client_info, req_params, time, cost, anticipation_timeout);
	   client.prop_delta = lowest_prop_tag - tmp_tag.proportion; 
	    // I think the purpose of doing this is to pull the delayed w tag of this idled client alined with the lowest tag among all other active clients.
	    // so delta is supposed to be  lowest_prop_tag - this_client.prev_tag
		do_file << "lowest_prop_tag is " << format_time(lowest_prop_tag,1000000) << " time is "<< format_time(time, 1000000) << " tmp_tag.proportion is "<< format_time(tmp_tag.proportion,1000000) << "\n";
		do_file << " Fire the idle condition. client.prop_delta is " << format_time(client.prop_delta,1000000) << "\n";
		const ClientInfo* client_info_i = get_cli_info(client);
		assert(client_info_i);
		do_file << "Idle client is " << client_info_i->reservation << client_info_i->weight << client_info_i->limit << "\n";
	  }
	  client.idle = false; // poping requests means this client is not idle anymore
...
// 		
	...
	if(first_idle) do_file << " first_idle \n";
            do_file << "new_tag:" << tag << "\n";
			...
			if(req_params.delta == 1){
	 		client.w_tag_counter ++;
			}
			else{
				client.w_tag_counter = client.w_tag_counter + 1 + req_params.delta;}
	// 		do_file << "W: "<< client.w_tag_counter<<" \n";
            do_file << "prev_tag:" << client.get_req_tag() << "\n";
            do_file << "new_tag:" << tag << "\n";
...
do_file << " **** Before Adjust **** \n";
do_file << " **** RESERVE  ****\n";
        for (auto c = resv_heap.data.begin(); c != resv_heap.data.end(); ++ c){
                const ClientInfo* tmp_client_info = get_cli_info(*(*c));
                do_file << "Iam: "<< tmp_client_info->reservation << tmp_client_info->weight << tmp_client_info->limit << "\n";
                if((*c)->has_request()){
                        do_file << "top_request_tag:" << (*(*c)).next_request().tag << "\n";
                }
                else{
                        do_file << "top client has no client\n";
                }
        }
        do_file << " **** WEIGHT  ****\n";
        for (auto c = ready_heap.data.begin(); c != ready_heap.data.end(); ++ c){
                const ClientInfo* tmp_client_info = get_cli_info(*(*c));
                do_file << "Iam: "<< tmp_client_info->reservation << tmp_client_info->weight << tmp_client_info->limit << "\n";
                if((*c)->has_request()){
                        do_file << "top_request_tag:" << (*(*c)).next_request().tag << "\n";
        ...
        do_file << " **** LIMIT  ****\n";
        for (auto c = limit_heap.data.begin(); c != limit_heap.data.end(); ++ c){
                const ClientInfo* tmp_client_info = get_cli_info(*(*c));
                do_file << "Iam: "<< tmp_client_info->reservation << tmp_client_info->weight << tmp_client_info->limit << "\n";
                if((*c)->has_request()){
                        do_file << "top_request_tag:" << (*(*c)).next_request().tag << "\n";
...
do_file.close();
	...
	std::ofstream file;
        file.open("/home/wcw/dmclock-log/pop_process_request-tag.txt", std::ios::app);
        if (file.bad()) {
                std::cout << "Can not open file" << "\n";
         }
	...
	if (top.has_request()) {
	  ...
	  assert(client_info);
	  if (now - win_start <= win_size){
//	file << "reservation "<< formate_time(client_info->reservation,1000000)<<"\n";
		...
		file << "Iam: R"<< client_info->reservation << client_info->weight << client_info->limit << "\n";
			...
			if(top.cur_delta == 1){
                        top.w_tag_counter ++;
                        file << "if top.cur_delta == 1 \n";
                }
                else{
                        top.w_tag_counter = top.w_tag_counter + top.cur_delta + 1;
                }
		...
		const ClientInfo* client_info = get_cli_info(top);
         	 assert(client_info);
		file << "top does not have any request\n";
		if(top.r_tag_counter < client_info->reservation * win_size){
			// top.r_tag_counter ++;
			if(top.cur_rho == 1){
                        top.r_tag_counter ++;
                        ...
                        top.r_tag_counter = top.r_tag_counter + top.cur_rho + 1;
			file << "top.r_tag_counter + top.cur_rho + 1;\n";
                }
			file << "R: "<< top.r_tag_counter<<"\n";
		...
		//	top.w_tag_counter ++;
			if(top.cur_delta == 1){
                        	...
                        	top.w_tag_counter = top.w_tag_counter + top.cur_delta + 1;
				file << "top.w_tag_counter = top.w_tag_counter + top.cur_delta + 1;\n";
                	}
			file << "W: "<< top.w_tag_counter<<" \n";
	...
	file << " **** After Demoting or Promoting. ****\n";
        file << " **** RESERVE  ****\n";
        for (auto c = resv_heap.data.begin(); c != resv_heap.data.end(); ++ c){
                const ClientInfo* tmp_client_info = get_cli_info(*(*c));
                file << "Iam: "<< tmp_client_info->reservation << tmp_client_info->weight << tmp_client_info->limit << "\n";
	...
	/*
	ClientRec& p_top = ready_heap.top();
	RequestTag p_tag = top.next_request().tag;
	const ClientInfo* p_client_info = get_cli_info(p_top);
        assert(p_client_info);
	file << "after demote, top_tag\t" << p_tag << "\n";
	file << "Demote: D"<< p_client_info->reservation << p_client_info->weight << p_client_info->limit << "\n";
	*/
	file.close();
	// process
	...
	std::ofstream do_file;
	do_file.open("/home/wcw/dmclock-log/do_next_request.txt", std::ios::app);
	
	if (do_file.bad()) {
    	std::cout << "Can not open file" << "\n";
 	}
	do_file<<"==============do_next_request=============\n";
...
*/
	...
	do_file << "**** After Demote  Limit Heap ****\n";
	  for (auto c = limit_heap.data.begin(); c != limit_heap.data.end(); ++ c){
                const ClientInfo* tmp_client_info = get_cli_info(*(*c));
                do_file << "Iam: "<< tmp_client_info->reservation << tmp_client_info->weight << tmp_client_info->limit << "\n";
		if((*c)->has_request()){
	...
	do_file << "**** After Promote Ready Heap****\n";
	for (auto c = ready_heap.data.begin(); c != ready_heap.data.end(); ++ c){
                const ClientInfo* tmp_client_info = get_cli_info(*(*c));
                do_file << "Iam: "<< tmp_client_info->reservation << tmp_client_info->weight << tmp_client_info->limit << "\n";
		do_file << "idle_delta " << (*c)->prop_delta << "\n";
		if((*c)->has_request()){
                        do_file << "top_request_tag:" << (*(*c)).next_request().tag << "\n";
                }
                else{
...
do_file.close();
	...
	std::ofstream pullfile;
        pullfile.open("/home/wcw/dmclock-log/pull.txt", std::ios::app);
        if (pullfile.bad()) {
                std::cout << "Can not open file" << "\n";
        }
        pullfile << "=============Enter pull_request================ \n" << format_time(now, 1000000) << "\n";
// here will demote limit and promote ready heap.
